...
- `somfy_rts_cover.yaml`: reusable package template (one virtual remote plus a "Pair" button plus the cover logic above), instantiated per cover via substitution variables (`cover_id`, `cover_name`, `remote_address`, `device_class`).
- `secrets.yaml` (gitignored): Wi-Fi credentials, API encryption key, OTA password. Example:
  ```yaml
  wifi_ssid: "your_wifi_name"
  wifi_password: "your_wifi_password"

  somfy_rts_bridge_api_key: "base64-encoded-32-byte-key"
  somfy_rts_bridge_ota_password: "some-password"
  ```
